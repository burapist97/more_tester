...
# --- SCRATCH / BLOCKLY CANLI CSS TASARIMI (SABİT KALDI) ---
st.markdown("""
    <style>
...
if 'bundle_id' not in st.session_state: st.session_state.bundle_id = "com.apple.Preferences"
...
with st.sidebar:
    ...
    out_filename = st.text_input("Çıktı Dosyası Adı:", "otomasyon_testi.py")
    ...
    if st.session_state.cases:
        ...
        step_name = st.text_input("Adım İsmi:", placeholder="Örn: Giriş butonuna bas")
        ...
        if st.button("⬇️ Aktif Case'e Adım Ekle", use_container_width=True):
            if not step_name:
                st.warning("Lütfen adıma bir isim verin!")
            else:
...
with col_canvas:
    ...
    for c_idx, case in enumerate(st.session_state.cases):
        ...
        for s_idx, step in enumerate(case["steps"]):
            ...
            html_block = f'<div class="s-block {css}"><span>{icon} <b>{s_name}</b> ({act})</span> {xp_disp} {info} {val_disp}</div>'
...
"""
    ...
    for case in st.session_state.cases:
        ...
        for s_idx, step in enumerate(case["steps"]):
            act = step["action"]
            s_name = step.get("step_name", f"Adım {s_idx+1}")
            gen_code += f"        print('Adım: {s_name} ({act})...')\n"
            ...
            if act == "Tıkla":
                ...
                gen_code += f"        kutu.clear(); kutu.send_keys('{step.get('val','')}'); time.sleep(1)\n"
    ...
    if not out_filename.endswith(".py"): out_filename += ".py"
    
    st.code(gen_code, language="python")
    st.download_button("📥 Kodu İndir (.py)", data=gen_code, file_name=out_filename, mime="text/x-python", use_container_width=True)